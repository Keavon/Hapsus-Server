...
var add_user = function () {
  ...
  for (var i=1;i<users.length;i++) { 
    if(users[i] === false && !is_user_placed) {
      is_user_placed = true;
      user_number = i;
      if(configuration.config.debug.messages) console.log("Found room in existing list");
    }
  }
  ...
  if(configuration.config.debug.messages) console.log("Added Player: " + user_number);
...
var remove_user = function(number) {
  if(configuration.config.debug.messages) console.log("Removed Player: " + number);
  users[number] = false;
};
